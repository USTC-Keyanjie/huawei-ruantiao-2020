...
int main()
{
    ...
    int div_scc_freq = 7000;     // divide scc every div_scc_freq, range in [1, id_num / 2]
    int div_scc_threshold = 80; // divide scc if num of nodes in scc more than div_scc_threshold, range in [2, id_num]
    ...
    for (div_scc_freq = 100; div_scc_freq <= id_num / 2; div_scc_freq+=50)
    {
        for (div_scc_threshold = 20; div_scc_threshold <= id_num; div_scc_threshold+=20)
        {
            clock_t search_time = clock();
            ...
            if (cur_time < rank[5][0])
            {
                rank[5][0] = cur_time;
                rank[5][1] = div_scc_freq;
                rank[5][2] = div_scc_threshold;
            }
    ...
    for (int i = 0; i < 5; i++)
    {
        cout << "cur_time " << rank[i][0] << " div_scc_freq " << rank[i][1] << " div_scc_threshold " << rank[i][2] << endl;
    }
